...
`tools/list` at `https://gateway.pipeworx.io/pride/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/pride_search_projects \
  -H 'Content-Type: application/json' \
  -d '{"keyword":"breast cancer"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/pride_search_projects`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
